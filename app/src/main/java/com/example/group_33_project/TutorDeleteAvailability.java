...
public class TutorDeleteAvailability extends AppCompatActivity {
    ...
    private DeleteAvailabilityAdapter adapter;
            ...
            public void onDelete(TimeSlot slot) {
                tutorH.deleteAvailability(slot, new TutorCallback() {
                    ...
                    public void onSuccess(String msg) {
                        runOnUiThread(() -> {
                            adapter.removeItem(slot);
                            Toast.makeText(TutorDeleteAvailability.this, "Deleted.", Toast.LENGTH_SHORT).show();
                        });
                    }
                    ...
                    public void onFailure(String msg) {
                        runOnUiThread(() ->
                                Toast.makeText(TutorDeleteAvailability.this, "Delete failed: " + msg, Toast.LENGTH_SHORT).show());
                    }
                });
            }
    ...
    private void loadSlots() {
        ...
        // Pull the common statuses tutors care about.
        String[] statusesToShow = new String[]{"open", "booked", "pending"};

        for (String status : statusesToShow) {
            tutorH.getAllSlotsByStatus(status, new SlotListCallback() {
                @Override
                public void onSuccess(List<TimeSlot> allTimeSlots) {
                    List<TimeSlot> mine = new ArrayList<>();
                    if (allTimeSlots != null) {
                        for (TimeSlot ts : allTimeSlots) {
                            if (ts == null || ts.getTutor() == null || ts.getTutor().getEmail() == null) continue;
                            if (!ts.getTutor().getEmail().equalsIgnoreCase(currentTutor.getEmail())) continue;

                            // Normalize to minute to avoid noisy seconds/nanos
                            ZonedDateTime s = ts.getStartDate() != null ? ts.getStartDate().truncatedTo(ChronoUnit.MINUTES) : null;
                            ZonedDateTime e = ts.getEndDate() != null ? ts.getEndDate().truncatedTo(ChronoUnit.MINUTES) : null;
                            if (s != null) ts.setStartDate(s);
                            if (e != null) ts.setEndDate(e);
                    ...
                    runOnUiThread(() -> {
                        slotList.addAll(mine);
                        slotList.sort(Comparator.comparing(TimeSlot::getStartDate, (a, b) -> {
                ...
                public void onFailure(String error) {
                    Log.e("TutorDeleteAvailability", "Failed to load slots (" + status + "): " + error);
                }
            });
        }
    }
